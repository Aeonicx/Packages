...
ROOTFS_DIR="${current_directory}/temp"
...
case $input in
    ...
    0)
    curl -o /tmp/rootfs.tar.xz "https://github.com/termux/proot-distro/releases/download/v3.10.0/debian-${ARCH}-pd-v3.10.0.tar.xz"
    ...
    2)
    curl -o /tmp/rootfs.tar.gz "https://dl-cdn.alpinelinux.org/alpine/v3.18/releases/x86_64/alpine-minirootfs-3.18.3-${ARCH}.tar.gz"
...
if [ ! -e $ROOTFS_DIR/.installed ]; then
    ...
    curl -o $ROOTFS_DIR/usr/local/bin/proot "https://raw.githubusercontent.com/Aeonicx/Packages/main/proot/proot-${ARCH}"
...
display_footer() {
	...
	echo -e ""
    echo -e "           ${YELLOW}-----> LINUX HAS STARTED <----${RESET_COLOR}"
}
